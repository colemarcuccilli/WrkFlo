  ...
  if (!driveConnected) {
    return (
      <div className="p-3 text-center">
        <p className="text-xs text-gray-500 mb-2">Connect Google Drive in Settings to import files</p>
        <a
          href="/settings"
    ...
    <div className="p-3">
      {importing ? (
        <div className="flex items-center justify-center gap-2 py-2">
          ...
          <span className="text-xs text-gray-500">Importing files...</span>
        </div>
      ) : (
        <button
          onClick={() => setShowPicker(true)}
          className="w-full flex items-center justify-center gap-2 px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-700 hover:bg-gray-50 transition-colors"
        >
