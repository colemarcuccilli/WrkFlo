...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const file of files) {
      const { id: onedriveFileId, name, mimeType } = file
      ...
      const { data: record, error } = await service
        ...
        .insert({
          ...
          version: 'V1',
          status: 'in-review',
          url: `/api/onedrive/proxy/${onedriveFileId}`,
          storage_type: 'onedrive',
          external_id: onedriveFileId,
          mime_type: mimeType,
      ...
      if (error) {
        console.error('Failed to import OneDrive file:', error.message)
        continue
      }
